...
typedef struct string {
    ...
    void* mass;
    //добавить размер символа!!!
    int sizeofchar;
} STRING;

void constructorString(STRING* s, const char* str, int sizec) {
    s->len = strlen(str) + 1;
    s->sizeofchar = sizec;
    s->mass = (char*) malloc(s->sizeofchar * s->len);
    strcpy(s->mass, str);
}

void constructorStringN(STRING* s, unsigned long long n, int sizec) {
    s->len = n;
    s->sizeofchar = sizec;
    s->mass = (char*) malloc(s->sizeofchar * s->len);
}
...
STRING* concat(STRING* s1, STRING* s2) {
    ...
    if (s1->sizeofchar != s2->sizeofchar)
        return NULL;
    constructorStringN(res, s1->len + s2->len - 1, s1->sizeofchar);
...
STRING* subString(const STRING* s, int i, int j) {
    if (i < 0 || j <= 0 || i >= j || j > s->len)
        return NULL;
    STRING* res = (STRING*) malloc(sizeof(STRING));
    constructorStringN(res, j - i, s->sizeofchar);
...
int main() {
    ...
    STRING A;
    constructorString(&A, buf, sizeof(char));
    ...
    switch(whattodo) {
        case 1:
            ...
            STRING B;
            constructorString(&B, buf2, sizeof(char));
        ...
        case 2:
            ...
            STRING C;
            constructorString(&C, buf3, sizeof(char));
